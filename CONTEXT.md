...
- **Remote bridge**: A per-SSH-session Unix-socket bridge owned by `ghosttykitd`, not a sidecar process. `ghosttykitd` creates a local bridge listener for the originating Ghostty surface, and `gty ssh` exposes that listener to the remote host with OpenSSH Unix-socket `RemoteForward`. It must not assume shpool or any other persistent remote shell manager. The default policy is one active bridge per local TTY; starting a second bridge for the same local TTY fails until the existing bridge is closed or explicitly removed. If remote forwarding is unavailable, `gty ssh` should warn and continue as plain SSH.
